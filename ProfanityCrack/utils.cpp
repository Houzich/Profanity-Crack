...
void closeOutStreams(std::ofstream out[]) {
	for (int i = 0; i < 256; i++)
	{
		try {
			out[i].close();
...
void readFromStream(std::ifstream& in, void* data, size_t size) {
	try {
		in.read((char*)data, size);
	}
	catch (std::ofstream::failure e) {
		std::cerr << "Error read from stream: " << e.what() << '\n';
		throw;
	}
}
...
void stringToPoint(std::string str, point* pubKey) {
	for (int i = 0; i < 8; i++) {
		...
		pubKey->x.d[7 - i] = (mp_word)std::stoull(substr, nullptr, 16);
		substr = str.substr(((size_t)i * 8 + 64), 8);
		pubKey->y.d[7 - i] = (mp_word)std::stoull(substr, nullptr, 16);
	}
}
...
private_key calcKeyFromResult(private_key privKey, result r) {
...
private_key calcKeyFromResult(private_key privKey, size_t round, size_t id_key) {
	...
	found.key.s[0] = privKey.key.s[0] + round; carry = found.key.s[0] < round;
	found.key.s[1] = privKey.key.s[1] + carry; carry = !found.key.s[1];
	found.key.s[2] = privKey.key.s[2] + carry; carry = !found.key.s[2];
	found.key.s[3] = privKey.key.s[3] + carry + id_key;
